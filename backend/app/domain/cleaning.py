...
def build_quality_report(articles: list[ArticleClean]) -> dict[str, dict[str, float]]:
    """Build a per-source and overall quality report for normalized articles."""
    totals = {
        "title": 0,
        "year": 0,
        "doi": 0,
    ...
    for article in articles:
        ...
        stats = source_totals.setdefault(
            source,
            {
                "title": 0,
                "year": 0,
    ...
    def normalize(stats: dict[str, int]) -> dict[str, float]:
        return {
            "title": _percent(stats["title"], stats["total"]),
            "year": _percent(stats["year"], stats["total"]),
            "doi": _percent(stats["doi"], stats["total"]),
            "abstract": _percent(stats["abstract"], stats["total"]),
            "duplicate_rate": _percent(stats["duplicates"], stats["total"]),
        }

    return {
        "overall": normalize(totals),
        "sources": {source: normalize(stats) for source, stats in source_totals.items()},
    }
